...
export default function MovieGridClient({ movies }: MovieGridClientProps) {
  ...
  const getEpisodeBadges = (movie: any) => {
    ...
    if (episodes.length > 0) {
      ...
      const isSongNgu = snEpisodes.length > 0;
      ...
      if (subEpisodes.length > 0) {
        ...
        badges.push({ label: 'PĐ.', count: maxEp, bg: 'bg-[#5E6070]/95', isSongNgu });
      }
      if (dubEpisodes.length > 0) {
        ...
        badges.push({ label: 'TM.', count: maxEp, bg: 'bg-[#2CA35D]/95', isSongNgu });
      }

      // if (isSongNgu) {
      //   badges.push({ label: 'Song Ngữ', count: null, bg: 'bg-gradient-to-r from-[#d9b8ff] to-[#9888ff] !text-[#1c1c1c]' });
      // }

      if (badges.length > 0) return { badges, isSongNgu };
    }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-x-3 gap-y-6">
        {movies.map((movie) => {
          ...
          return (
            ...
            >
              <Link className="movie-card block group" href={detailLink}>
                {/* Poster Image Container */}
                <div className="relative rounded-lg overflow-hidden bg-[#12121a] aspect-[2/3] ring-1 ring-white/5 shadow-md flex items-center justify-center border border-white/5">
                  {/* Fallback film roll icon */}
                  ...
                  <img
                    ...
                    src={getProxyImageUrl(getPosterUrl(movie), 256)}
                  />
                  ...
                  {/* IMDb Rating Overlay */}
                  {movie.imdb_rating && (
                    <span className="absolute bottom-2 left-2 z-20 text-[10px] px-1.5 py-[3px] rounded-md font-semibold bg-[#01b4e4] text-white shadow-sm flex items-center gap-0.5">
                      ★ {Number(movie.imdb_rating).toFixed(1)}
                    </span>
                  )}

                  {/* Episode counts bottom banner */}
                  <div className="absolute inset-x-0 bottom-0 flex items-end pb-1.5 md:pb-0 pointer-events-none justify-center z-20">
                    <div className="flex w-full flex-col md:flex-row md:flex-nowrap gap-1 md:gap-0 items-start md:items-end justify-start pl-1 sm:pl-2 md:pl-0 md:justify-center">
                      {badges.map((badge, bidx) => (
                        ...
                        >
                          <span className="font-normal md:font-bold">{badge.label}</span>
                          {badge.count !== null && <span className="font-bold"> {badge.count}</span>}
                        </span>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Hover Overlay */}
                  <div className="overlay absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10" />
                </div>

                {/* Texts below card */}
                <div className="text-center px-1 mt-2">
                  <h3 className="text-[12px] sm:text-[13px] font-semibold text-white truncate group-hover:text-[#FECF59] transition-colors">
                    {movie.title}
                  </h3>
        ...
        >
          <div className="bg-[#1f2129] rounded-xl overflow-hidden shadow-2xl shadow-black/60 border border-white/10">
            <div className="relative aspect-video w-full overflow-hidden flex items-center justify-center">
              <svg className="absolute w-12 h-12 text-white/10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-1/4 bg-gradient-to-t from-[#1f2129] to-transparent z-20" />
            </div>
            <div className="p-4 pt-2">
              ...
              <div className="flex items-stretch gap-2 mt-3 h-[36px]">
                <Link href={`/phim/${hoveredMovie.slug}`} className="flex-[1.2] flex items-center justify-center gap-1.5 px-0 text-[#1c1c1c] bg-[#FFD166] hover:bg-[#ffdf8f] rounded-lg text-[13px] font-bold transition-all shadow-md shadow-[rgba(254,207,89,0.2)] active:scale-95">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5c0-.8.9-1.3 1.6-.9l9 6c.6.4.6 1.4 0 1.8l-9 6c-.7.4-1.6-.1-1.6-.9V5z" /></svg>
                  Xem ngay
                </Link>
              ...
              <p className="text-xs text-zinc-500 mt-2.5 line-clamp-1">{hoveredMovie.description ? hoveredMovie.description.replace(/<[^>]*>?/gm, '') : 'Khoa Học • Hành động'}</p>
            </div>
          </div>
        </div>
      )}
