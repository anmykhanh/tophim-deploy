...
export default function RegisterForm({ logoUrl, siteName, bgMovies, googleClientId }: RegisterFormProps) {
  ...
  return (
    <div className="relative w-full min-h-screen flex justify-center items-center py-10 px-4 bg-[#08080a] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[420px] bg-[#0f1118]/80 backdrop-blur-[20px] border border-white/[0.06] rounded-[24px] p-8 sm:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.7)] text-center text-white before:absolute before:top-0 before:left-1/4 before:right-1/4 before:h-[1px] before:bg-gradient-to-r before:from-transparent before:via-[#00ac47]/50 before:to-transparent">
        <div className="mb-8">
          <h2 className="text-[26px] font-bold text-white mb-2 tracking-tight">Đăng ký tài khoản</h2>
          <p className="text-sm text-zinc-400">Khám phá thế giới điện ảnh không giới hạn cùng Tô Phim.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-user absolute left-4 text-zinc-400 w-4 h-4 pointer-events-none"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
            <input 
              type="text" 
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full py-3.5 rounded-xl bg-[#00ac47] hover:bg-[#00c250] text-white font-bold text-[15px] transition-all shadow-[0_4px_15px_rgba(0,172,71,0.2)] hover:shadow-[0_6px_20px_rgba(0,172,71,0.35)] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 !mt-6"
          >
            {isLoading ? (
              <>
        ...
        <div className="mt-8 text-sm text-zinc-400">
          Đã có tài khoản?
          <Link href="/pages/login" className="text-white font-bold hover:text-[#00c250] ml-1 transition-colors">
            Đăng nhập ngay
          </Link>
        </div>
      </div>
