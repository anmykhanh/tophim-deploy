...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ category?: string; country?: string; year?: string; search?: string }> }): Promise<Metadata> {
  ...
  let title = 'Phim Lẻ - Xem Phim Lẻ Hay Nhất | Tô Phim';
  let description = 'Xem phim lẻ chất lượng cao FHD, 4K, vietsub mới nhất. Phim lẻ Hàn, Mỹ, Âu Đồng Cập nhật liên tục tại Tô Phim.';

  if (categoryId) {
    try {
      const category = await prisma.categories.findUnique({ where: { id: categoryId } });
      if (category) {
        title = `Phim Lẻ ${category.name} - Xem Online Miễn Phí | Tô Phim`;
        description = `Xem phim lẻ ${category.name} chất lượng cao, vietsub mới nhất tại Tô Phim.`;
      }
    } catch {}
  } else if (params.search) {
    title = `Tìm kiếm phim lẻ: "${params.search}" | Tô Phim`;
    description = `Kết quả tìm kiếm phim lẻ cho từ khóa "${params.search}" tại Tô Phim.`;
  }
...
export default async function PhimLePage({ searchParams }: PageProps) {
  ...
  if (currentCategory) {
    pageTitle = `Phim Lẻ - ${currentCategory.name}`;
  } else if (search) {
    pageTitle = `Phim Lẻ - Tìm kiếm: "${search}"`;
  }
