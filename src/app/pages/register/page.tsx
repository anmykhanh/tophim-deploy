...
export default async function RegisterPage() {
  ...
  // Fetch logo, site name and google client id from settings
  ...
  try {
    const settingsList = await prisma.settings.findMany({
      where: {
        key: {
          in: ['logo_url', 'site_name', 'google_client_id']
        }
      }
    });
    settingsList.forEach(s => {
      ...
      if (s.key === 'google_client_id' && s.value) googleClientId = s.value;
    });
  } catch (err) {
  ...
  return (
    <RegisterForm logoUrl={logoUrl} siteName={siteName} bgMovies={bgMovies} googleClientId={googleClientId} />
  );
}
