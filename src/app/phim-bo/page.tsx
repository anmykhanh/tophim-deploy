...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ category?: string; country?: string; year?: string; search?: string }> }): Promise<Metadata> {
  const params = await searchParams;
  const categoryId = params.category ? parseInt(params.category, 10) : undefined;
  let title = 'Phim Bộ - Xem Phim Bộ Hay Nhất | Tô Phim';
  let description = 'Xem phim bộ chất lượng cao, vietsub, thuyết minh mới nhất. Cập nhật phim bộ Hàn Quốc, Trung Quốc, Mỹ, Nhật liên tục tại Tô Phim.';

  if (categoryId) {
    try {
      const category = await prisma.categories.findUnique({ where: { id: categoryId } });
      if (category) {
        title = `Phim Bộ ${category.name} - Xem Online Miễn Phí | Tô Phim`;
        description = `Xem phim bộ ${category.name} chất lượng cao, vietsub mới nhất tại Tô Phim.`;
      }
    } catch {}
  } else if (params.search) {
    title = `Tìm kiếm phim bộ: "${params.search}" | Tô Phim`;
    description = `Kết quả tìm kiếm phim bộ cho từ khóa "${params.search}" tại Tô Phim.`;
  }
...
function getEpisodeBadges(movie: any) {
  ...
  if (count !== null) {
    ...
    badges.push({ label: 'PĐ.', count: 1, bg: 'bg-[#5E6070]/95' });
  }
  return badges;
}
...
export default async function PhimBoPage({ searchParams }: PageProps) {
  ...
  // Compute title
  let pageTitle = 'Phim Bộ';
  if (currentCategory) {
    pageTitle = `Phim Bộ - ${currentCategory.name}`;
  } else if (search) {
    pageTitle = `Phim Bộ - Tìm kiếm: "${search}"`;
  }
